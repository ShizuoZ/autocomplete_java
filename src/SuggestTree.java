...
public class SuggestTree {
    ...
    public int height(Node x){
    	if (x == null) return 0;
    	return 1 + Math.max(Math.max(height(x.left), height(x.mid)),height(x.right));
    }
    ...
    public int treeSize(Node x){
    	if(x == null) return 0;
    	return 1 + treeSize(x.left) + treeSize(x.mid) + treeSize(x.right);
    }
...
//      int k = Integer.parseInt(args[1]);
        ...
        while (sc.hasNextLine()) {
            ...
            Node resultNode = tree.getAutocompleteSuggestions(prefix);
            for (int i = 0; i < Math.min(k, resultNode.list.length); i++) 	//Math.min(k, results.length)
                System.out.println(resultNode.list[i]);
            System.out.println("Used memory: " + (instance.totalMemory() - instance.freeMemory()) / mb + "MB");
            System.out.println("search time: " + (numCompare - runtime));
            runtime = numCompare;
        }
        System.out.println("Used memory: " + (instance.totalMemory() - instance.freeMemory()) / mb + "MB");
        System.out.println("Number of compares: " + SuggestTree.numCompare);
        sc.close();
    }
}